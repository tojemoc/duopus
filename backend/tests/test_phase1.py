...
@pytest.mark.asyncio
async def test_seed_creates_admin_template_and_today_rundown(tmp_path):
    os.environ["DATABASE_URL"] = f"sqlite+aiosqlite:///{tmp_path}/duopus_test.db"
    ...
    async with SessionMaker() as session:
        ...
        rd = (
            await session.execute(
                select(models_mod.Rundown).where(models_mod.Rundown.template_id == tmpl.id, models_mod.Rundown.show_date == date.today())
            )
        ).scalars().first()
